...
def edit_offsite_config(request):
    if request.method == 'GET':
        ...
        return render_to_response('bkp/edit/edit_offsite.html',return_dict, context_instance=RequestContext(request))
...
def enable_offsite(request):
    if request.method == 'POST':
        vars_dict, forms_dict, return_dict = global_vars(request)
        vars_dict['gconfig'] = get_object_or_404(GlobalConfig, pk=1)
        forms_dict['offsiteform'] = OffsiteConfigForm(request.POST, instance=vars_dict['gconfig'])
        if forms_dict['offsiteform'].is_valid():
            forms_dict['offsiteform'].save()
            return HttpResponseRedirect(utils.edit_offsite_config_path(request))
        else:
            return_dict = utils.merge_dicts(return_dict, forms_dict, vars_dict)
            return render_to_response('bkp/edit/edit_offsite.html',return_dict, context_instance=RequestContext(request))
