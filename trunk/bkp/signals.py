...
### NetworkInterface ###
def update_iftab_file():
	interfaces = NetworkInterface.objects.all()
	generate_iftab('iftab', interfaces)

	
def generate_iftab(filename, interfaces):
	f = utils.prepare_to_write(filename, 'custom/network')

	# TODO: Criar função para estampar comentário de generated
	f.write("# Control file for ifrename tool\n")
	f.write("# Needed for renaming interfaces\n")
	f.write("# Generated by Nimbus\n")
	for iface in interfaces:
		f.write("%s\t\t%s %s\n" % (iface.interface_name, 'mac', iface.interface_mac))
	f.close()

def update_interfaces_file():
	interfaces = NetworkInterface.objects.all()
	generate_interfaces('interfaces', interfaces)
	
def generate_interfaces(filename, interfaces):
	f = utils.prepare_to_write(filename, 'custom/network')
	
	# TODO: Criar função para estampar comentário de generated
	f.write("# Interfaces config\n")
	f.write("# Generated by Nimbus\n")
	f.write("auto lo\n")
	...
	for iface in interfaces:
		...
		f.write("\t\tnetwork %s\n" % iface.interface_network)
		f.write("\t\tgateway %s\n" % iface.interface_gateway)
